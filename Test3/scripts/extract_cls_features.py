"""
Extract CLS features from a trained FiLM+Spatial MoE model.
Outputs a per-episode CLS embedding map for TRMDIS usage.
"""
import sys
...
def main():
    ...
    ckpt_path = data_dir / "film_smoe_transformer.pt"
    out_path = data_dir / "film_smoe_cond_features.npz"
    ...
    with torch.no_grad():
        for i in range(0, n_samples, 512):
            batch = torch.tensor(X_norm[i:i + 512], dtype=torch.float32, device=device)
    ...
    cls_out = np.concatenate(cls_out_list, axis=0).astype(np.float32)
    fourier_seq = np.concatenate(fourier_seq_list, axis=0).astype(np.float32)
    router_logits = np.concatenate(router_logits_list, axis=0).astype(np.float32)
    np.savez(
        out_path,
